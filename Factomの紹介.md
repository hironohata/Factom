# **Factomの紹介**
本稿では、Off-Blocks がバックエンドに使用しているFactomについて、その特徴やメリットなどを紹介する。

初版　2021年3月7日

## 1. はじめに　
Off-Blocks は、バックエンドにブロックチェーンを使用している。ブロックチェーンを使用することにより、以下のメリットがある。
...
 
## 3．サプライチェーン アプリケーション XXXX での位置づけ

Factom ブロックチェーンは、サプライチェーンのアプリケーション XXXX から Off-Blocks デジタルID・署名・証明管理サブシステム経由でアクセスされる。その状況を下に示す。
